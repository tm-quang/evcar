...
export const vehicleKeys = {
    ...
    notifications: ['vehicle_notifications'] as const,
}

import { fetchVehicleNotifications } from './vehicleNotificationService'

// 0. Hook for vehicle notifications
export function useVehicleNotifications() {
    return useQuery({
        queryKey: vehicleKeys.notifications,
        queryFn: fetchVehicleNotifications,
...
export function useVehicleStats(vehicleId: string | undefined, startDate?: string, endDate?: string) {
    return useQuery({
        queryKey: [...vehicleKeys.stats(vehicleId || ''), startDate, endDate],
        queryFn: () => getVehicleStats(vehicleId!, startDate, endDate),
        enabled: !!vehicleId,
...
export function useVehicleCharging(vehicleId: string | undefined) {
    return useQuery({
