...
import { createVehicleNotification } from './vehicleNotificationService'

const applyArchiveFilter = (query: any, _dateField?: string) => query;
...
export async function createVehicle(vehicle: Omit<VehicleRecord, 'id' | 'user_id' | 'created_at' | 'updated_at'>): Promise<VehicleRecord> {
    ...
    await createVehicleNotification({
        ...
        message: `Đã thêm xe ${data.license_plate} (${data.model}) vào danh sách quản lý.`,
        related_id: data.id
    })
...
export async function createFuelLog(fuelLog: Omit<FuelLogRecord, 'id' | 'user_id' | 'created_at' | 'updated_at'>): Promise<FuelLogRecord> {
    ...
    await createVehicleNotification({
        ...
        message: `Đã ghi nhận phiên sạc ${fuelLog.kwh || 0} kWh cho xe tại ${fuelLog.odometer_at_refuel} km.`,
        related_id: data.id
    })
...
export async function createMaintenance(maintenance: Omit<MaintenanceRecord, 'id' | 'user_id' | 'total_cost' | 'created_at' | 'updated_at'>): Promise<MaintenanceRecord> {
    ...
    await createVehicleNotification({
        ...
        message: `Đã ghi nhận bảo dưỡng tại mốc ${maintenance.odometer} km. Nội dung: ${maintenance.service_items.join(', ')}`,
        related_id: data.id
    })
