...
function TripNodeItem({ node, prevNode }: { node: TripTimelineNode; prevNode: TripTimelineNode | null }) {
    const [resolvedAddress, setResolvedAddress] = useState<string | null>(null)

    useEffect(() => {
        if (node.lat && node.lng) {
            const isCoordStr = node.address && /^-?\d+\.?\d*,\s*-?\d+\.?\d*$/.test(node.address)
            if (!node.address || isCoordStr) {
                reverseGeocode(node.lat, node.lng).then(addr => {
    ...
    }, [node.lat, node.lng, node.address])
    ...
    const displayAddress = resolvedAddress || node.address
    ...
    return (
        <div className="flex gap-4 items-start">
            ...
            <div className="flex-1 bg-slate-50 border border-slate-100 rounded-xl p-3 shadow-sm relative overflow-hidden">
                {resolvedAddress && (
                    <div className="absolute top-0 right-0 bg-blue-100 text-blue-600 text-[9px] font-bold px-1.5 py-0.5 rounded-bl-lg rounded-tr-xl opacity-70">
                        Auto
                    </div>
                )}
                ...
                {displayAddress && (
                    <p className="text-[13px] font-medium text-slate-700 leading-snug mb-2 pr-4">
                        {displayAddress}
                    </p>
                )}
    ...
    return (
        <div className="mt-3 relative">
            ...
            <div className="space-y-4 relative z-10">
                {nodes.map((node, i) => (
                    <TripNodeItem key={i} node={node} prevNode={i > 0 ? nodes[i - 1] : null} />
                ))}
            </div>
        </div>
    )
